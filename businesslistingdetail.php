...
$result = mysqli_query($con,"SELECT * FROM biimages where idd=$var");

while($row = mysqli_fetch_array($result))
{
echo '
   <div class="bpbItem"><img src="images/bi/' . $row['file_name'] . '" alt="post_11"></div>
...
$result = mysqli_query($con,"SELECT * FROM bicategory WHERE id=$var");
while($row = mysqli_fetch_array($result))
{
 date_default_timezone_set('America/New_York');  

$address = ''.$row['street'].', '.$row['area'].', '.$row['landmark'].', '.$row['taluka'].', '.$row['city'].' - '.$row['pincode'].' ';
...
echo '   
                                        ...
                                        <header class="header btClear extralarge">
                                            ...
                                            <div style="    font-size: 2.125em;" class="btArticleExcerpt">'.$row['businessname'].'</div>
                                        </header>

  <br> Hours of Operation - '.$row['hours'].' 
...
<br>
  <span class="btIco btIcoFilledType btIcoSmallSize btIcoFacebook">
                                              ...
                                              <span class="btIco btIcoFilledType btIcoSmallSize btIcoTwitter"><a href="http://twitter.com/share?text='.$row['businessname'].'&url=http://amarprerana.org/nammauk/newsdetailspage.php?q='.$_GET['q'].'" target="_blank" data-ico-fa="&#xf099;" class="btIcoHolder"></a></span>
  ...
  <a style="background-color: #25d366;
    border-radius: 50px;color:white" href="whatsapp://send?text=http://amarprerana.org/nammauk/newsdetailspage.php?q='.$_GET['q'].'" target="_blank" data-ico-fa="&#xf232;" class="btIcoHolder"></a>
                                            ...
                                            <div class="bt_bb_wrapper">
                                                ...
                                                <p> <i style="font-size:20px;color:#dc0003;margin-right:8px" class="fa fa-mobile" aria-hidden="true"></i> '.$row['mobile'].' 
  <br>                                                            
  <i style="font-size:16px;color:#dc0003;margin-right:3px" class="fa fa-chrome" aria-hidden="true"></i> '.$row['website'].'
  <br> 

   <i style="font-size:20px;color:#dc0003;margin-right:8px" class="fa fa-map-marker" aria-hidden="true"></i> 
'.$row['street'].', '.$row['area'].', '.$row['landmark'].', '.$row['taluka'].', '.$row['city'].' - '.$row['pincode'].' </p>
  
                                              <br>
<iframe style="width:500px;height:100px" frameborder="0" src="https://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q=' . str_replace(",", "", str_replace(" ", "+", $address)) . '&z=14&output=embed"></iframe> 
