...
if ($db->connect_error) {
    ...
    if(!empty(array_filter($_FILES['files']['name']))){
        ...
        if(!empty($insertValuesSQL)){
            ...
            $insert = $db->query("INSERT INTO articleimages (idd,file_name, uploaded_on) VALUES $insertValuesSQL");
            if($insert){
...
if(isset($_GET['del']))
{
    $SQL = $con->query("DELETE FROM articleimages WHERE id=".$_GET['del']);
...
$result = mysqli_query($con,"SELECT * FROM articleimages where idd=$abc");
...
echo '     <tbody> <tr> 
    ...
    <td><img width="200px" src="../../images/articles/'.$row['file_name'].'" />  
...
<?php $conn = new PDO ('mysql:host=localhost; dbname=nammauk', 'root', ''); ?>
  <?php
                            if(isset($_FILES['file'])){
                                ...
                                {
                                    move_uploaded_file($tmp, '../../images/articles/'.$random_name.'.'.$extension);    
                                  //  $conn->query("insert into videos (title,location) values ('$name','$random_name.$extension')");
                                     $conn->query("insert into articlevideos (idd,file_name) values ('$idd','$random_name.$extension')");
...
if(isset($_GET['del']))
{
    $SQL = $con->query("DELETE FROM articlevideos WHERE id=".$_GET['del']);
...
/* code for data delete */
$result = mysqli_query($con,"SELECT * FROM articlevideos where idd=$abc");
